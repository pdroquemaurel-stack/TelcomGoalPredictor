...
export default async function AdminChallengesPage() {
  const [competitions, challenges] = await Promise.all([
    ...
    prisma.challenge.findMany({ include: { competitions: { include: { competition: true } }, _count: { select: { fixtures: true } } }, orderBy: { createdAt: 'desc' } }),
  ]);
  ...
  return (
    <div className="space-y-4 text-black">
      ...
      <form action={upsertChallenge} className="grid gap-2 rounded-2xl bg-white p-4 md:grid-cols-2">
        ...
        <select multiple name="competitionIds" className="rounded border p-2 md:col-span-2" required>
      ...
      <div className="overflow-auto rounded-2xl bg-white p-4">
        <table className="w-full text-sm">
          <thead><tr className="text-left"><th>Nom</th><th>Compétitions</th><th>Période</th><th>Statut</th><th>Matchs</th><th>Action</th></tr></thead>
          <tbody>
